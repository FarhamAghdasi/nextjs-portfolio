                        ...
                        {skills.map((skill: Skill, index: number) => (
                            <div key={index} className={`col-lg col-md-4 col-6`}>
                                <div className={`item ${index < skills.length - 1 ? "md-mb30" : ""}`}>
                                    <div className="box">
                                        <div className="img">
                                            <Image src={skill.image} alt={skill.name} width={64} height={64} objectFit="contain" />
                                        </div>
                                        <h2>{skill.level}</h2>
                                    </div>
                                    <h6>{skill.name}</h6>
                        ...
                        {numbers.map((num: NumberItem, idx: number) => (
                            <div key={idx} className="col-lg-3">
                                <div className={`item ${idx === numbers.length - 1 ? "" : "mb-60"}`}>
                                    ...
                                    <p>
                                        {num.link ? (
                                            <a href={num.link} rel="nofollow" target="_blank" >
                                                {num.label}
                                            </a>
                                        ) : (
                                            num.label
                    ...
                    <div className="slide-har st1 d-flex">
                        {[0, 1].map((boxIdx) => (
                            <div key={boxIdx} className="box">
                                {marquee.slice(boxIdx * 5, boxIdx * 5 + 5).map((item, i) => (
