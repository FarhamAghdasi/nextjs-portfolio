...
const BlogInfo: React.FC<BlogInfoProps> = ({ post, posts }) => {
  ...
  return (
    <>
      <SEO
        title={currentTitle}
        description={post.short_description || texts.defaultDescription}
                    ...
                    <div className="author-info">
                      <div className="d-flex align-items-center">
                        <a href="#" className="circle-60">
                          <Image src={authorImage} alt={texts.authorAlt} className="circle-img" width={60} height={60} />
                        </a>
                      ...
                      <div>
                        <div className="img">
                          <Link href={`/blog/${p.url}`}>
                            <Image
                              src={`https://farhamaghdasi.ir${p.thumbnail}`}
                              alt={p.title}
                              width={80}
                              height={80}
                            />
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = postsData.posts.map((post: PostDetails) => ({
    params: { url: post.url },
  }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const post = postsData.posts.find((p: PostDetails) => p.url === params?.url) || null;
